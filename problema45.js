...
async function consult(){
    let data = fs.readFileSync('DATOS.DAT', 'utf-8');
    let registros = JSON.parse(data);
    let criterio = await read("Ingrese el ID del registro a consultar: ");
    let index = -1;
    for (let i = 0; i < registros.length; i ++){ //sacamos la posición del objeto JSON si es que existe
        let reg = registros[i];
        if (reg["id"] == criterio ){
    ...
    if(index != -1){
        ...
        console.log("Error: no existe el registro con el id especificado.")
    }
    await menu();
};
...
function printMenu(){
    ...
    console.log("4.- Consultar por ID.")
